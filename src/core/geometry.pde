...
public class Vector2 implement Cloneable {
...
public class Point2 extends Vector2 {
  ...
  public Point2 rotate(Point2 pivot, float a) {
    return sub(pìvot).rotate(a).add(pivot);
  }
}

public class Rectangle2 implement Cloneable {
  public final Point2 p;
  public final Vector2 dim;
  public Mark(Point2 p, Vector2 dim) {
    this.p = p;
  }
  public Mark clone() {
    return this;
  ...
  public Rectangle2 scale(Point2 pivot, float nx, float ny) {
    Vector2 np = p.scale(pivot, nx, ny);
  ...
  public Rectangle2 round() {
    ...
    if (cx<c.x) cx++;
    if (cy<c.y) cy++;
    Point2 np = point(px, py);
    Point2 nc = point(cx, cy);
  ...
  public Rectangle2 merge(Rectangle2... rects) {
    ...
    return new Rectangle2(point(px,py), vector(cx-px,cy-py));
  }
  //FIXME: add intersect()
}

// Some shortcuts
public Point2 point(float x, float y) {
  return new Point2(x,y);
}
public Point2 point(Vector2 v) {
  return new Point2(v);
}
